...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            codeforces_username = form.cleaned_data['codeforces_username']
            registered = True
            user_profile = UserProfile(user=user, uva_id=uva_id, codeforces_username=codeforces_username)
...
def profile(request):
    form = PostForm()
    oj_info = OJStanding.objects.all().order_by('-total_submission')
    if request.method == 'POST':
...
def standing(request):
    return render(request, 'App_Login/standing.html', context={})